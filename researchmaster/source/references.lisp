...
(defmethod process (s (file (eql 'displayAuthorLatex)) postlines)
  (let (dum dum2)
    (cond ((and (setq dum (getf-post-all "object" postlines)) (setq dum (mapcar #'read-user-string dum)))
	   (setq dum2 (getf-post "maxlen" postlines))
	   (if dum2
	       (setq dum2 (read-user-string dum2))
	       (setq dum2 t))
	   (process-displayauthorlatex s dum dum2))
          (t (process-displaybibtex s (instances 'paper *gui*))))))

(defmethod process (s (file (eql 'displayAuthorBibtex)) postlines)
  (let (dum)
    (cond ((and (setq dum (getf-post-all "object" postlines)) (setq dum (mapcar #'read-user-string dum)))
	   (process-displaybibtex s (find-papers-for-authors dum)))
          (t (process-displaybibtex s (instances 'paper *gui*))))))

(defun find-papers-for-authors (authors &key (requirepub nil))
  (setq authors (mapcan #'(lambda (auth) (viewfinds '?x `(paper.author ?x ,auth) *repository*)) authors))
  (when requirepub (setq authors (delete-if-not #'paper-publishedp authors)))
  authors)

(defun paper-publishedp (paper) (viewfindp `(or (paper.publication ,paper ?x) (paper.publisher ,paper ?x)) *repository*))
...
(defun process-displaylatex (s papers &optional (maxauthlength 4))
  ...
  (dolist (p (sortem papers (find-sorter 'paper) 'descending))
    (when (doublep 'paper.instance p *repository*)
      (format s "\\item ")
      (output-paper-latex p s :shortenauthor maxauthlength)
      (format s "~%")))
  (format s "\\end{enumerate}~%")
  (format s "</pre>")
  (finish-body s) (crlf s)
  (finish-html s) (crlf s))
...
(defun output-paper-html-source (handle bibtex booktitle publication volume number startpage endpage publisher year s)
  (declare (ignore handle))
  (let (prefix)
    (setq prefix (compute-pub-prefix bibtex))
    (cond (booktitle (setq publication (stringappend prefix booktitle)))
	  (publication (let ((short (prorequest `(ask-one ?x (shortname ,publication ?x)))))
			 (setq publication (stringappend prefix (iconify publication)))
    ...
    (when publisher (format s "                ~A" (iconify publisher)))
    (when (eq bibtex 'techreport) (format s " Technical report~A~A" (if number " " "") (if number number "")))
    (when (or publisher (eq bibtex 'techreport)) (format s "~A" (if year "," ".")))
    (when year (format s "                <span class=\"date\">~A</span>.~%" year))
    (format s "            </span>~%")
    (format s "")))
...
(defun output-paper-latex (handle s &key (minimal nil) (shortenauthor t) )
  ...
  (let (author title year bibtex booktitle publisher publication volume number startpage endpage  (*namer* 'prettyname) award)
    ...
    (cond ((and shortenauthor (or (and (not (numberp shortenauthor)) (> (length author) 4)) 
				  (and (numberp shortenauthor) (> (length author) shortenauthor))))
...
(defmethod output-paper-bibtex-body (handle (bibtex (eql 'techreport)) s)
  (let (num)
    (setq num (result 'paper.number handle *repository*))
    ; required fields
    (output-paper-bibtex-authors (results 'paper.author handle *repository*) s)
    (output-paper-bibtex-comma s)
    (output-paper-bibtex-value 'title (result 'paper.title handle *repository*) s)
    ...
    (output-paper-bibtex-comma s)
    (when num
      (output-paper-bibtex-value 'number num s)
      (output-paper-bibtex-comma s))
    (output-paper-bibtex-value 'year (result 'paper.year handle *repository*) s)))
