...
(defun fhlp (x builtins)
  "(FHLP X BUILTINS) returns a list of reasons that X is not in FHL."
  (let (es tmp)
    (cond ((member x '(true false)) nil)
	  ((atom x) (list (cons x "The only permitted propositions are TRUE and FALSE.")))
	  ((not (listp x)) (list (cons x "Every formula must be either an atom or a list.")))
	  ((not (atom (car x))) (list (cons x "First element of every list must be an atom.  Did not analyze remainder of list.")))
	  ((member (first x) '(and or => <= <=>)) (mapcan #'(lambda (y) (fhlp y builtins)) (cdr x)))
	  ((eq (first x) 'not) 
	   (setq es (mapcan #'(lambda (y) (fhlp y builtins)) (cdr x)))
	   (if (cddr x) (cons (cons x "NOT only takes 1 argument but analyzed all arguments.") es) es))
	  ((member (first x) '(forall exists))
	   (setq es (fhlp (third x) builtins))
	   (if (not (or (varp (second x)) (and (listp (second x)) (every #'varp (second x)))))
	       (cons (cons x "Second argument to Quantifier must either be a variable or a list of variables") es)
	       es))
	  ((setq tmp (remove-if-not #'(lambda (b) (eq (first x) (parameter-symbol b))) builtins))
	   (setq es (mapcan #'(lambda (y) (fhl-termp y builtins)) (cdr x)))
	   ; check if there is *any* builtin that is a relation with the appropriate arity for (first x)
	   (let (isfunc isreln ispropreln)
	     (dolist (b tmp)
	       (when (isrelation b) 
		 (setq isreln t)
		 (when (or (not (parameter-arity b)) (= (parameter-arity b) (1- (length x))))
		   (setq ispropreln t)
		   (return)))
	       (when (isfunction b)
		 (setq isfunc t)))
	     (cond (ispropreln es)
		   (isreln (cons (cons x "There is a builtin relation by this name but of a different arity.") es))
		   (isfunc (cons (cons x "This atom occurs at the formula level, but there is only a builtin function by this name.") es))
		   (t (cons (cons x "This atom utilizes a builtin that is neither a function nor relation.") es)))))
	  (t (mapcan #'(lambda (y) (fhl-termp y builtins)) (cdr x))))))

(defun fhl-termp (x builtins)
  (let (tmp es)
    (cond ((not (or (numberp x) (symbolp x) (stringp x) (listp x))) 
	   (list (cons x "Every term must be atomic (symbol, string, number) or a list")))
	  ((atom x) nil)
	  ((member (first x) '(and or not => <= <=> forall exists)) 
	   (list (cons x "Operators (and, or, not, <=, =>, <=>, forall, exists) cannot appear as terms.  Did not analyze subformulas.")))
	  ((setq tmp (remove-if-not #'(lambda (b) (eq (first x) (parameter-symbol b))) builtins))
	   (setq es (mapcan #'(lambda (y) (fhl-termp y builtins)) (cdr x)))
	   (let (isfunc isreln ispropfunc)
	     (dolist (b tmp)
	       (when (isfunction b) 
		 (setq isfunc t)
		 (when (or (not (parameter-arity b)) (= (parameter-arity b) (1- (length x))))
		 ...
		 (setq isreln t)))
	     (cond (ispropfunc es)
		   (isfunc (cons (cons x "There is a builtin function by this name but of a different arity.") es))
		   (isreln (cons (cons x "This atom occurs at the term level, but there is only a builtin relation by this name.") es))
		   (t (cons (cons x "This atom utilizes a builtin that is neither a function nor relation.") es)))))
	  (t (cons (cons x "All functions must be builtins.") (mapcan #'(lambda (y) (fhl-termp y builtins)) (cdr x)))))))
