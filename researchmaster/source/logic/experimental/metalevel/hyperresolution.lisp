...
(defun hyperresolution (sos background)
  "(HYPERRESOLUTION SOS BACKGROUND) runs hyperresolution using BACKGROUND
   as the set of positive clauses and SOS as the set of other clauses.  Returns T
  ...
  (let ((b nil) (s nil) binfo sinfo)
    ...
    (when (and (not (clausesetinfo-unsat binfo)) (not (clausesetinfo-unsat sinfo)))
      (hyperresolution-aux s b)) )
)

(defun hyperresolution-aux (pos nonpos)
  "(HYPERRESOLUTION-AUX POS NONPOS) returns T if clauses POS U NONPOS is unsatisfiable.
   Does resolutions among the NONPOS and between the POS and NONPOS but not among the POS."
  (let ((poslist (make-queue :head pos :tail (last pos)))
        (nonposlist (make-queue :head nonpos :tail (last pos)))   ; B
        (intposlist nil)     ; A
        (intnonposlist (make-queue :head nonpos :tail (last pos)))  ; 
        )
...
(defun metaresolution-aux (sos background)
  "(METARESOLUTION-AUX SOS BACKGROUND) returns T if clauses SOS U BACKGROUND are inconsistent.
   Else returns nil.  Enqueues positive facts in background and  "
...
(defun hyperresolution-aux (sos background)
  "(METARESOLUTION-AUX SOS BACKGROUND) returns T if clauses SOS U BACKGROUND are inconsistent.
   Else returns nil. "
  (let ((pos (make-queue :head background :tail (last background)))
        (active (make-queue :head sos :tail (last sos)))
        (newresolvents (make-queue))
        (result nil)
        (allstop nil)
        )
    ...
    (do ((level 1 (1+ level))
      ...
      ; choose the clause to make active
      (setq chosen (choose-next-active pos))

      ; compute all resolvents of chosen with active
      (setq info (metaresolution-aux-allpairs (list chosen) (queue-head active)))
      (when (clausesetinfo-unsat info) (return t))
            
      ; if no new resolvents, exit
      (when (null (queue-head newresolvents)) (return nil))

      ; add new resolvents to the appropriate queue
      (dolist (c (clausesetinfo-clauses info))
